Groestlcoin Core version 29.0 is now available from:
...
- libnatpmp was replaced with a built-in implementation of PCP and NAT-PMP (still enabled using the `-natpmp` option). This supports automatic IPv4 port forwarding as well as IPv6 pinholing.

- When the `-port` configuration option is used, the default onion listening port will now
be derived to be that port + 1 instead of being set to a fixed value (1334 on mainnet).
...
previously Groestlcoin Core would listen for incoming Tor connections on `127.0.0.1:1334`.
Now it would listen on `127.0.0.1:5556` (`-port` plus one). If you configured the hidden service manually
in torrc now you have to change it from `HiddenServicePort 1331 127.0.0.1:1334` to `HiddenServicePort 1331
127.0.0.1:5556`, or configure groestlcoind with `-bind=127.0.0.1:1334=onion` to get the previous behavior.

- Upon receiving an orphan transaction (an unconfirmed transaction that spends unknown inputs), the node will attempt to download missing parents from all peers who announced the orphan. This change may increase bandwidth usage but make orphan-handling more reliable.
...
The minimum value of `-blockreservedweight` is set to `2,000 WU`. Users setting `-blockreservedweight` below the default should ensure that the total weight of their block header, transaction count, and coinbase transaction does not exceed the reduced value or they may risk mining an invalid block.
...
- the `getblocktemplate` RPC `curtime` (BIP22) and `mintime` (BIP23) fields now
  ...
  miners will not accidentally violate the timewarp rule.
...
- The maximum allowed value for the `-dbcache` configuration option has been
  ...
  values were automatically reduced to 16 GiB (1 GiB on 32 bit systems).
...
replace-by-fee the standard behavior.

- Setting `-upnp` will now log a warning and be interpreted as `-natpmp`. Consider using `-natpmp` directly instead.

- As a safety check, Groestlcoin core will **fail to start** when `-blockreservedweight` init parameter value is lower than `2000` weight units. Groestlcoin Core will also **fail to start** if the `-blockmaxweight` or `-blockreservedweight` init parameter exceeds consensus limit of `4,000,000 WU`.
...
- The default for `-rpcthreads` has been changed from 4 to 16, and the default for `-rpcworkqueue` has been changed from 16 to 64.
...
3. CMake variables may be used to configure the build system. **Some defaults have changed.** For example, you will now need to add `-DWITH_ZMQ=ON` to build with zmq and `-DBUILD_GUI=ON` to build `groestlcoin-qt`. See [Autotools to CMake Options Mapping](https://github.com/bitcoin-core/bitcoin-devwiki/wiki/Autotools-to-CMake-Options-Mapping) for details.
4. For single-configuration generators, the default build configuration (`CMAKE_BUILD_TYPE`) is "RelWithDebInfo". However, for the "Release" configuration, CMake defaults to the compiler optimization flag `-O3`, which has not been extensively tested with Groestlcoin Core. Therefore, the build system replaces it with `-O2`.
...
cmake --install build --component groestlcoind
```
...
- A new tool [`utxo_to_sqlite.py`](https://github.com/groestlcoin/groestlcoin/blob/29.0.0/contrib/utxo-tools/utxo_to_sqlite.py)
