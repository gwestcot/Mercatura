...
This version of Groestlcoin Core adds a new `mempoolfullrbf` configuration
...
Some Groestlcoin services today expect that the first version of an
...
The Groestlcoin Protocol does not, and cannot, provide any assurance that
...
fees, was realized in Groestlcoin Core 2.13.3 (January 2017) with the
...
default for years, and several contributors to Groestlcoin Core are
...
- The `-deprecatedrpc=softforks` configuration option has been removed.  The
  ...
  previously deprecated in 23.0. Information on soft fork status is
...
- The `deprecatedrpc=fees` configuration option has been removed. The top-level
  ...
  The top-level fee fields were previously deprecated in 23.0.
...
- The `sendall` RPC spends specific UTXOs to one or more recipients
  ...
  `send`, `sendtoaddress`, or `sendmany` RPCs.

- A new `gettxspendingprevout` RPC has been added, which scans the mempool to find
  transactions spending any of the given outpoints.
...
- The `simulaterawtransaction` RPC iterates over the inputs and outputs of the given
  ...
  inputs that the wallet will then sign for unintentionally.
...
- The `wsh()` output descriptor was extended with Miniscript support. You can import Miniscript
  ...
  the Groestlcoin Core wallet yet.
...
- The `tr()` output descriptor now supports multisig scripts through the `multi_a()` and
  `sortedmulti_a()` functions.

- To help prevent fingerprinting transactions created by the Groestlcoin Core wallet, change output
...
- RPC `getreceivedbylabel` now returns an error, "Label not found
  in wallet" (-4), if the label is not in the address book.
...
[documentation](https://github.com/groestlcoin/groestlcoin/blob/master/doc/managing-wallets.md#migrating-legacy-wallets-to-descriptor-wallets).
...
- Configuration changes made in the groestlcoin GUI (such as the pruning setting,
proxy settings, UPNP preferences) are now saved to `<datadir>/settings.json`
file rather than to the Qt settings backend (windows registry or unix desktop
config files), so these settings will now apply to groestlcoind, instead of being
ignored.

- Also, the interaction between GUI settings and `groestlcoin.conf` settings is
simplified. Settings from `groestlcoin.conf` are now displayed normally in the GUI
settings dialog, instead of in a separate warning message ("Options set in this
dialog are overridden by the configuration file: -setting=value"). And these
settings can now be edited because `settings.json` values take precedence over
`groestlcoin.conf` values.
