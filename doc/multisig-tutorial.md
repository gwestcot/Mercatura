...
Currently, it is possible to create a multisig wallet using Groestlcoin Core only.
...
Before starting this tutorial, start the groestlcoin node on the signet network.

```bash
./build/bin/groestlcoind -signet -daemon
```
...
At the time of writing, there is no way to extract a specific path from wallets in Groestlcoin Core. For this, an external signer/xpub can be used.
...
for ((n=1;n<=3;n++))
do
 ./build/bin/groestlcoin rpc -signet createwallet "participant_${n}"
done
```

Extract the xpub of each wallet. To do this, the `listdescriptors` RPC is used. By default, Groestlcoin Core single-sig wallets are created using path `m/44'/1'/0'` for PKH, `m/84'/1'/0'` for WPKH, `m/49'/1'/0'` for P2WPKH-nested-in-P2SH and `m/86'/1'/0'` for P2TR based accounts. Each of them uses the chain 0 for external addresses and chain 1 for internal ones, as shown in the example below.
...
All RPCs in Groestlcoin Core will include the checksum in their output.
...
for ((n=1;n<=3;n++))
do
 xpubs["xpub_${n}"]=$(./build/bin/groestlcoin rpc -signet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/0/*") )][0] | .desc' | grep -Po '(?<=\().*(?=\))' | sed 's /0/\* /<0;1>/* ')
done
```
...
checksum=$(./build/bin/groestlcoin rpc -signet getdescriptorinfo $desc | jq -r '.checksum')
...
Documentation for these and other parameters can be found by typing `./build/bin/groestlcoin rpc -signet help importdescriptors`.
...
```bash
./build/bin/groestlcoin rpc -signet createwallet "multisig_wallet_01" disable_private_keys=true blank=true

./build/bin/groestlcoin rpc -signet -rpcwallet="multisig_wallet_01" importdescriptors "$multisig_desc"

./build/bin/groestlcoin rpc -signet -rpcwallet="multisig_wallet_01" getwalletinfo
```
...
```bash
for ((n=1;n<=3;n++)); do ./build/bin/groestlcoin rpc -signet loadwallet "participant_${n}"; done
./build/bin/groestlcoin rpc -signet loadwallet "multisig_wallet_01"
```
...
The url used by the script can also be accessed directly. At time of writing, the url is [`https://signetfaucet.groestlcoin.org`](https://signetfaucet.groestlcoin.org).
...
```bash
receiving_address=$(./build/bin/groestlcoin rpc -signet -rpcwallet="multisig_wallet_01" getnewaddress)

./contrib/signet/getcoins.py -c ./build/bin/groestlcoin-cli -a $receiving_address
```
...
```bash
./build/bin/groestlcoin rpc -signet -rpcwallet="multisig_wallet_01" getbalances
```
...
PSBT is a data format that allows wallets and other tools to exchange information about a Groestlcoin transaction and the signatures necessary to complete it. [[source](https://bitcoinops.org/en/topics/psbt/)]

The current PSBT version (v0) is defined in [BIP 174](https://github.com/bitcoin/bips/blob/master/bip-0174.mediawiki).

For simplicity, the destination address is taken from the `participant_1` wallet in the code above, but it can be any valid groestlcoin address.
...
```bash
balance=$(./build/bin/groestlcoin rpc -signet -rpcwallet="multisig_wallet_01" getbalance)

amount=$(echo "$balance * 0.8" | bc -l | sed -e 's/^\./0./' -e 's/^-\./-0./')

destination_addr=$(./build/bin/groestlcoin rpc -signet -rpcwallet="participant_1" getnewaddress)

funded_psbt=$(./build/bin/groestlcoin rpc -signet -rpcwallet="multisig_wallet_01" walletcreatefundedpsbt outputs="{\"$destination_addr\": $amount}" | jq -r '.psbt')
```
...
```bash
./build/bin/groestlcoin rpc -signet decodepsbt $funded_psbt

./build/bin/groestlcoin rpc -signet analyzepsbt $funded_psbt
```
...
```bash
psbt_1=$(./build/bin/groestlcoin rpc -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq '.psbt')

psbt_2=$(./build/bin/groestlcoin rpc -signet -rpcwallet="participant_2" walletprocesspsbt $funded_psbt | jq '.psbt')
```
...
```bash
combined_psbt=$(./build/bin/groestlcoin rpc -signet combinepsbt "[$psbt_1, $psbt_2]")
```
...
```bash
finalized_psbt_hex=$(./build/bin/groestlcoin rpc -signet finalizepsbt $combined_psbt | jq -r '.hex')

./build/bin/groestlcoin rpc -signet sendrawtransaction $finalized_psbt_hex
```
...
```bash
psbt_1=$(./build/bin/groestlcoin rpc -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq -r '.psbt')

psbt_2=$(./build/bin/groestlcoin rpc -signet -rpcwallet="participant_2" walletprocesspsbt $psbt_1 | jq -r '.psbt')

finalized_psbt_hex=$(./build/bin/groestlcoin rpc -signet finalizepsbt $psbt_2 | jq -r '.hex')
