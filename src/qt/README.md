This directory contains the source code for the Groestlcoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Groestlcoin UI.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a groestlcoin:... link from a browser).
...
* UI elements like GroestlcoinAmountField, which inherit from QWidget.
* `bitcoinstrings.cpp`: automatically generated
* `bitcoinunits.(h/cpp)`: GRS / mGRS / etc. handling
* `callback.h`
...
8. While in the `Projects` tab, ensure that you have the `groestlcoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `build/bin`, and select `groestlcoin-qt`
9. You're all set! Start developing, building, and debugging the Groestlcoin Core GUI
