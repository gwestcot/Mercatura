...
AddressBookPage::AddressBookPage(const PlatformStyle *platformStyle, Mode _mode,
    ...
    : QDialog(parent), ui(new Ui::AddressBookPage), model(nullptr), mode(_mode),
      tab(_tab) {
    ui->setupUi(this);
    ...
    switch (tab) {
        case SendingTab:
            ui->labelExplanation->setText(
                tr("These are your Mercatura addresses for sending payments. "
        ...
        case ReceivingTab:
            ui->labelExplanation->setText(
                tr("These are your Mercatura addresses for receiving payments. "
...
void AddressBookPage::done(int retval) {
    ...
    QModelIndexList indexes =
        table->selectionModel()->selectedRows(AddressTableModel::Address);
...
void AddressBookPage::on_exportButton_clicked() {
    ...
    if (!writer.write()) {
        QMessageBox::critical(this, tr("Exporting Failed"),
                              tr("There was an error trying to save the "
                                 "address list to %1. Please try again.")
                                  .arg(filename));
    }
}
...
void AddressBookPage::selectNewAddress(const QModelIndex &parent, int begin,
                                       int /*end*/) {
    QModelIndex idx = proxyModel->mapFromSource(
        model->index(begin, AddressTableModel::Address, parent));
    if (idx.isValid() &&
        (idx.data(Qt::EditRole).toString() == newAddressToSelect)) {
