...
enum class FeeRateFormat {
    ...
    SAT_VB,  //!< Use gro/vB fee rate unit
};

/**
 * Fee rate in gros per virtualbyte: CAmount / vB
 * the feerate is represented internally as FeeFrac
 */
class CFeeRate
{
private:
    /** Fee rate in gro/vB (gros per N virtualbytes) */
    FeePerVSize m_feerate;

public:
    /** Fee rate of 0 gros per 0 vB */
    CFeeRate() = default;
    ...
    /**
     * Construct a fee rate from a fee in gros and a vsize in vB.
     *
    ...
    /**
     * Return the fee in gros for the given vsize in vbytes.
     * If the calculated fee would have fractional gros, then the
     * returned fee will always be rounded up to the nearest gro.
     */
    ...
    /**
     * Return the fee in gros for a vsize of 1000 vbytes
     */
